...
import { addEnrolledCourse } from "../redux/userSlice.js";
...
setIsEnrolled(true)
dispatch(addEnrolledCourse(courseId)); // ✅ add course ID to Redux
  ...
  return (
     <div className="min-h-screen bg-gray-50 ">
      <Nav/>
      <div className="max-w-6xl mt-20 mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative">
          ...
          <div className="flex-1 space-y-2 mt-[20px]">
            ...
            <div className="flex items-start flex-col justify-between">
              <div className="text-yellow-500 font-medium">
                ⭐ {avgRating} <span className="text-gray-500"></span>
              </div>
              <div>
                <span className="text-lg font-semibold text-black">{selectedCourseData?.price}</span>{" "}
                <span className="line-through text-sm text-gray-400">{selectedCourseData?.price + 1000}</span>
              </div>
            </div>
            ...
            {userData.user.role === "admin"  && <button className="bg-green-200 text-green-600 px-6 py-2 rounded hover:bg-gray-100 hover:border mt-3" onClick={()=>navigate(`/viewlecture/${courseId}`)}>
            Admin Watch Now
            </button>}

             {userData.user.role === "instructor" &&  userData.user._id === selectedCourseData?.creator  && <button className="bg-green-200 text-green-600 px-6 py-2 rounded hover:bg-gray-100 hover:border mt-3" onClick={()=>navigate(`/viewlecture/${courseId}`)}>
             Watch Now
            </button>} */}


{/* If ADMIN → Always watch */}
...
/* If INSTRUCTOR is course creator → Watch without enroll */
(userData.user.role === "instructor" &&
 userData.user._id === selectedCourseData?.creator) ? (
  <button
    className="bg-green-200 text-green-600 px-6 py-2 rounded hover:bg-gray-100 hover:border mt-3"
    onClick={() => navigate(`/viewlecture/${courseId}`)}
  >
...
/* Normal user → Enroll or Watch after enrolled */
(!isEnrolled ? (
  <button
    className="bg-[black] text-white px-6 py-2 rounded hover:bg-gray-700 mt-3"
    onClick={() => handleEnroll(courseId, userData.user._id)}
  >
    Enroll Now
  </button>
) : (
