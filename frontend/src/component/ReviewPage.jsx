...
function ReviewPage() {
  ...
  return (
     <div className='flex items-center justify-center flex-col'>
      ...
      <span className='lg:w-[50%] md:w-[80%] text-[15px] text-center mt-[30px] mb-[30px] px-[20px] text-[#6AA9FF]'>
    ...
    '>
     ...
     
            {
                latestReview.map((item,index)=>(
                    <ReviewCard key={index} rating={item.rating} image={item.user.photoUrl} text={item.comment} name={item.user.name} title={item?.course?.title} comment={item.comment} role={item.user.role} />
                ))
            }
